...
OnPeople Desktop 0.30.0 使用以下开源组件。各组件的完整许可证和版本以 Cargo.lock、package-lock.json 及其上游仓库为准。

- Tauri、Tauri plugins、wry、tao：MIT。
- Rust CEF bindings/runtime：BSD-3-Clause 及 Chromium 相关许可证；CEF 分发包必须随目标平台许可证一起发布。
...
任何未随仓库发布的专有浏览器 Profile 导入器、云端服务或签名材料均不属于本仓库的分发内容。
